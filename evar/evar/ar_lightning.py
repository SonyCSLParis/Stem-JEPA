...
class ARLightning(BaseAudioRepr):
    ...
    def _init_weights(self, weight_file: str | Path) -> None:
        ...
        for k, v in state_dict.items():
            if k.startswith('encoder.'):
                if k[8:].startswith("_orig_mod."):
                    encoder_state_dict[k[18:]] = v
                else:
    ...
    def precompute(self, device, data_loader):
        self.norm_stats = calculate_norm_stats(device, data_loader, self.to_feature, norm_stats=self.cfg.norm_stats)
